...
const PartIcon = ({ part }: { part: keyof SegmentResponse }) => {
  ...
  if (part === 'hands') {
    return (
      <svg className="w-[70%] h-[70%] opacity-60" fill="currentColor" viewBox="0 0 45.975 45.975" xmlns="http://www.w3.org/2000/svg">
        <g>
...
export default function SegmentResults({ results, selectedPart, onPartClick }: SegmentResultsProps) {
  const ENABLED_PART: keyof SegmentResponse = 'upper'; // 目前只支持身体部位识别
    ...
    <div className="w-full">
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
        {partOrder.map((part) => {
          ...
          const isEnabled = part === ENABLED_PART;
